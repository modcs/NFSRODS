...
bool NFS_RODS_PRIVATE_COLL = TRUE;
 ...
 * (uNFS v3) code, and newly added for our purposes.
 */
...
/**
 * @brief getUID returns the UID of the logged NFS Client
 * @param arg
 * @return
 */
...
/**
 * @brief getGID returns the group id of the logged NFS client
 * @param arg
 * @return
 */
...
/**
 * @brief getNFSDirectory returns the iRODS directory
 * @param arg
 * @return
 */
...
/**
 * @brief getRealPathNFS translates the user path to the server path
 * @param userPath
 * @param arg
 * @return
 */
char* getRealPathNFS(char *userPath,caddr_t arg){
    debug("getRealPathNFS %s",userPath);
...
char* getIRODSpathGetAttr(char * path,int uid){
    ...
    if(isPublic()){
        return substring(path,(strlen(export_path)+1),strlen(path));
    }
                                    ...
                                    struct svc_req * rqstp)
{
    ...
    if (result.status == NFS3_OK){
        ...
        int resultUser = getUserName(getUID(rqstp->rq_clntcred),userName);

        if(resultUser<0){
            debug("SET ATTR - Cant Find User", "");
    ...
    if (fd != -1) {
        ...
        if (res != -1) {
            ...
            if (argp->how.mode == EXCLUSIVE) {
                ...
                res = backend_store_create_verifier(obj, argp->how.createhow3_u.verf);
            }
        }
...
REMOVE3res *nfsproc3_remove_3_svc(REMOVE3args * argp, struct svc_req * rqstp)
{
    ...
    debug("try to Remove the collection %s in iRODS", name);
